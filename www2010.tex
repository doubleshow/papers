...
applications. Users can learn to perform a wide variety of tasks from these resources
such as how to set up a home network,
back up files, or change the speed of the mouse cursor. These
resources are often created and made available online by software
developers who wish to maintain an online version of
the documentation apart from the built-in one in order to keep the
content up-to-date (e.g., support.microsoft.com). These
...
searching, and organizing images on the web. Many researchers shared
...
diversification. Kennedy and Naaman \cite{Kennedy} focused on how to
generate diverse and representative search results for landmark
...
tried to improve the ranking of product images using PageRank.  The
...
images. Li et al \cite{Li} focused on improving the relevance
judgement of web search results with image excerpts, identifying the
dominant image in a page and displaying it in the excerpt.  Authority
identification problem. Lempel and Soffer \cite{Lempel} built an image
retrieval system; by analyzing link structure, they determine whether
an image is came from an authoritative source without using visual
...
and temporal features. In relation to these works, ours depends on
state-of-the-art visual analysis techniques to index and search
...
learners to engage in multi-lingual search to find the usage of any
...
information written in different languages, mining multilingual
topics from Wikipedia. In relation to these works, this work
offers another interesting possibility---searching based on the
universal visual language of screenshots.

There is also focus on helping computer users. Medhi et al \cite{Medhi}
studied the optimal way of presenting audio-visual know-how knowledge to
novice computer users.

Several works have been presented to enable users to search in more
than one modality. Narayan et al \cite{Narayan} studied multi-modal
interfaces for mobile devices for accessing web information through a 
personalized dialog based interaction based on speech and text. 
Dowman et al \cite{Dowman} studied the problem of indexing and
searching radio and television news using speech recognition transcripts
and meta data of the web pages containing related news. Our multi-modal
search is to use image and keywords with faceted search capabilities.

Amazon mechanical turk. In several components of the current work, we
applied AMT for both training and testing.  Recently, many works have
used AMT for labeling training images and for evaluating
systems. Kittur et al \cite{Kittur} experimented with AMT
